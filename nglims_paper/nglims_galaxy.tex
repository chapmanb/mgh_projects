...
\begin{abstract}
  ...
  It addresses an urgent need among small research support core facilities to easily and rapidly process high throughput
...
Once available only through large genome sequencing centers, next-generation 
sequencing machines have become accessible to a multitude of new users. Among these are
sequencing core facilities and individual labs who require
flexible interfaces for entering samples and tracking sequencing runs. Often, these 
users lack the computational expertise to efficiently process the large and sometimes complex data 
produced by these sequencing instruments.
Scaling to handle increasing sequencing throughput requires the
utilization of automated processing pipelines. The output from these
pipelines should make the results available to biologists in a format
that can be readily queried and visualized.

High throughput sequencing operations face 3 basic data challenges. First, 
acquisition of accurate sample information from the end-user. Examples of 
this type of data includes sample name, sample preparation details, type
of work required, and possibly method of payment. The second challenge is
tracking and managing this information as samples move through the 
different steps performed in the sequencing lab. Typically, it is useful to track 
when the sample is received, results of any quality control assays performed, and 
when and how the sample was actually sequenced. The last and perhaps greatest 
challenge is to rapidly return data in an intuitively useful format to the biologist end-user. 
This includes sequencing performance metrics demonstrating the quality of the sequencing
data as well as processed sequence data. Processing typically entails performing a 
genome alignment and providing the data to the user
in a format compatible with a genome browser such as the UCSC Genome Browser.
...
available for further analysis and visualization by biologists. In addition, 
this solution also facilitates analysis of sequencing lab performance metrics.
...
A table shows raw cluster and read counts for each sample, with optional
export functionality.
Interactive plots of run results over
time and pass rates versus read density reveal longer term performance trends and
expose individual outlier runs. These allow adjustment of
...
5). Data are only accessible by the user account that submitted the work
request, maintaining data privacy. Users can download results for offline work, or import them
...
sample submission, depending on the sequencing
...
\begin{itemize}
  \item De-multiplex barcoded samples sequenced in a pool into separate samples, combining
...
increasingly common. With the continuing increase in the volume of biological 
data, both through next generation sequencing and future high throughput 
